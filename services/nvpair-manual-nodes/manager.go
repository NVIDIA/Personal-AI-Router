...
// llamaCppPort is llama.cpp's own default server port, probed the same way LM
// Studio is hardcoded to 1234. A manual node is remote, so (like the other
// engines) we assume the engine's default port rather than resolving it via the
// engine manager, which only governs the local engine.
//
// 8080 answers for either kind of peer, which is why it is the right guess: a
// bare llama.cpp listens there by default, and a peer running PAIR has its
// llama.cpp facade there with the engine relocated above it. Probing the
// relocated port instead would reach the engine directly and bypass routing.
