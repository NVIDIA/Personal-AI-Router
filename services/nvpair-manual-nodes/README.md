...
A Go service for managing manually configured nodes on networks where mDNS discovery is unavailable. Accepts node addresses via JSON-RPC, probes each for Ollama, LM Studio, llama.cpp, and node-info, and emits status events.
...
- **llama.cpp** on port 8082: `GET /v1/models`, which doubles as the liveness check and the loaded-model list (`status.value == "loaded"`)
