...
func waitForPriorityPair(t *testing.T, ch <-chan jsonrpc.Message, timeout time.Duration) map[string]schedulerwire.EnginePriority {
	...
	for {
		...
		case msg, ok := <-ch:
			...
			if _, ok := engines.ByName(p.Engine); ok {
				got[p.Engine] = p
			}
			if len(got) == len(engines.Names()) {
				return got
			}
		case <-to:
			t.Fatalf("timed out (%s) waiting for a schedule:priority output per engine (%v); got %v",
				timeout, engines.Names(), got)
		}
	}
}
...
func waitForSchedulePair(t *testing.T, ch <-chan jsonrpc.Message, timeout time.Duration) map[string][]string {
	...
	for _, engine := range engines.Names() {
		out[engine] = priorities[engine].Nodes
	}
	return out
}
...
func TestSchedulerSyntheticMixedEngineBurst(t *testing.T) {
	...
	for i := 0; i < 50; i++ {
		target := current[0]
		// Rotate through every engine: the ranking is node-global, so load
		// attributed to one engine must move the others' order too.
		names := engines.Names()
