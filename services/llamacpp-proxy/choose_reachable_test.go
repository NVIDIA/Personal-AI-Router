...
func TestChooseReachableFailsOverForPinnedPeer(t *testing.T) {
	...
	if u := p.targetURL(n); u == nil || u.Host != net.JoinHostPort("192.0.2.10", "8081") {
		t.Fatalf("first selection = %v, want the published ranking without waiting", u)
	}
	waitForTarget(t, p, n, net.JoinHostPort(reachable, "8081"))
	if dials.Load() != 2 {
...
func TestNodeCandidatesKeepsPublishedOrder(t *testing.T) {
	...
	want := []string{
		net.JoinHostPort("10.172.55.129", "8081"),
		net.JoinHostPort("192.168.240.1", "8081"),
	}
