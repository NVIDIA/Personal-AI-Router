...
// proxyBin is the real llamacpp-proxy binary, built once in TestMain so the
// e2e test exercises the shipped artifact (not just in-process handlers).
var proxyBin string

func TestMain(m *testing.M) {
	tmp, err := os.MkdirTemp("", "nvpair-lcpproxy-e2e-*")
	if err != nil {
		panic(err)
	}
	suffix := ""
	...
	proxyBin = filepath.Join(tmp, "llamacpp-proxy"+suffix)
	if out, err := exec.Command("go", "build", "-o", proxyBin, ".").CombinedOutput(); err != nil {
		panic("build llamacpp-proxy: " + err.Error() + "\n" + string(out))
	}
	code := m.Run()
	_ = os.RemoveAll(tmp)
	os.Exit(code)
...
// TestE2EFailoverOverRealBinary spawns the real llamacpp-proxy binary and
