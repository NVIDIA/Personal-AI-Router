...
// (discovery:nodes snapshots for the lc service) into the subscribed overlay,
// merged with user-added manual nodes. The proxy is itself advertised — as an lc
...
type Node struct {
	...
	TXT       []string `json:"txt"`
	// Models is the loaded-model set from the broker's discovery snapshot
	// (EngineLoadedModels), not the on-disk catalog. Model-bearing inference is
	// eligible only when this list advertises the requested model; an empty
	// list stays in discovery but is not an inference candidate until a later
	// loaded-set update. GET /v1/models still merges the full backend catalog.
...
type Discovery struct {
	...
	// relay (discovery:nodes snapshots for the lc service), keyed by node ID (the
