...
func TestSubscribedToNode(t *testing.T) {
	withLM := noderec.DirectoryNode{
		...
		Services: map[noderec.ServiceKey]noderec.ServiceStatus{
			noderec.ServiceLlamaCpp: {Port: 8081},
		},
	}
	...
	if got.ID != "uuid-a" || got.Port != 8081 || got.IP != "10.0.0.5" ||
	...
	// Per-engine attribution: a dual-engine node projects ONLY its llama.cpp
...
func TestSubscribedToNodeKeysByHostUUID(t *testing.T) {
	...
	n := noderec.DirectoryNode{
		...
		Name:     "host-a",
		IP:       "10.0.0.5",
		Services: map[noderec.ServiceKey]noderec.ServiceStatus{noderec.ServiceLlamaCpp: {Port: 8081}},
	}
