...
// which for streaming llama.cpp responses is dominated by token
...
const (
	...
	// workload this proxy produces. This proxy only ever fronts llama.cpp.
	workloadEngine = "llamacpp"
)
...
// non-inference noise. llama.cpp serves the OpenAI-compatible API, so these
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	// for lc nodes; they arrive as discovery:nodes snapshots (handled in
	...
	slog.Debug("subscribing to discovery relay for routing targets", "service", string(noderec.ServiceLlamaCpp))
	if err := p.codec.Notify(noderec.MethodSubscribe, noderec.SubscribeParams{Services: []noderec.ServiceKey{noderec.ServiceLlamaCpp}}); err != nil {
	...
	// connection — so the target llama.cpp sees the client disconnect and stops
...
func (retrySignal) Error() string { return "llamacpp-proxy: retry next candidate" }
...
// serveModelList queries every llama.cpp candidate concurrently and returns
...
// (self-forward guard): a local llama.cpp advertisement could otherwise
...
func (p *Proxy) resolveCandidates(model string) []candidate {
	...
	add := func(n Node) {
		...
		case isSelfTarget(u, selfPort):
			// Our own advertised endpoint (lc now points at this proxy). Serve
		...
		case p.mesh.HasPin(n.ClusterUUID):
			...
			// proxy (the lc port now advertises the proxy, not the engine).
...
// discovery:nodes snapshot: it projects every node advertising lc with a dialable
...
func upstreamUnreachableID(nodeID string) string {
	return "llamacpp-proxy:upstream-unreachable:" + nodeID
}

// subscribedToNode projects a relay DirectoryNode onto the proxy's routable Node
// for the lc service, returning false when the node doesn't advertise lc or has
// no dialable address. The engine port comes from the lc service key (the real
// llama.cpp port the broker's engine poller registered, not the proxy's listen
// port).
func subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	...
	return Node{
		...
		// Routing eligibility uses the loaded set, not the on-disk catalog, so a
		// catalog-only id is never treated as a llama.cpp owner. Dual-engine
		// nodes still project only this engine's loaded models, never the union.
