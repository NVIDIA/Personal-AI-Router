...
func subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	...
	return Node{
		...
		// Filter on this node's llama.cpp models only, not the cross-engine union,
		// so a model a dual-engine node serves solely via Ollama isn't accepted as
		// an llama.cpp owner here (falls back to the union for a peer that sends
		// no attribution — see DirectoryNode.EngineModels).
		Models: append([]string(nil), n.EngineModels("llamacpp")...),
	}, true
}
