...
func TestDefaultProxyPort(t *testing.T) {
	if defaultProxyPort != 8081 {
		t.Fatalf("default proxy port = %d, want 8081", defaultProxyPort)
	}
}
...
func TestChooseStartupPort(t *testing.T) {
	for _, tc := range []struct {
		...
		{"new default", 8081, 0, false, false, 8081},
		{"legacy default migrates", 8081, 8080, false, true, 8081},
		{"custom survives opt-out", 8081, 12400, false, true, 12400},
		{"managed flag wins", 8081, 12400, true, true, 8081},
	} {
		t.Run(tc.name, func(t *testing.T) {
			if got := chooseStartupPort(tc.flagPort, tc.ignorePersisted, tc.persisted, tc.hasPersisted); got != tc.want {
