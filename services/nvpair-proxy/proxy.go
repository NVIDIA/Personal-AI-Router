...
const (
	...
	// overrides it. Engines send no headers until generation starts, so a
	// request queued behind other work, or waiting on a slow model load,
	// needs longer than this to survive the proxy.
...
//
// It is a var (not a const) so a test can shorten it, and so main can set it
// from the resolved --response-header-timeout at startup; it always tracks
