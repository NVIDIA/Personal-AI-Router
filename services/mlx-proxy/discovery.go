...
type Node struct {
	...
	// Loaded is the subset of Models the node currently holds in memory.
	// mlx_lm.server keeps exactly one model resident and reloads from scratch
	// when asked for another, so residency -- not mere availability on disk --
	// is what decides which owner should take a request. Empty means "nothing
	// resident" or "the peer did not report residency"; both are handled the
	// same way, as "not preferred", never as "not eligible".
