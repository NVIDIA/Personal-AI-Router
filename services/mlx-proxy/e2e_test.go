...
// proxyBin is the real mlx-proxy binary, built once in TestMain so the
...
func TestMain(m *testing.M) {
	...
	proxyBin = filepath.Join(tmp, "mlx-proxy"+suffix)
	if out, err := exec.Command("go", "build", "-o", proxyBin, ".").CombinedOutput(); err != nil {
		panic("build mlx-proxy: " + err.Error() + "\n" + string(out))
	}
	code := m.Run()
	_ = os.RemoveAll(tmp)
	os.Exit(code)
...
// TestE2EFailoverOverRealBinary spawns the real mlx-proxy binary and
