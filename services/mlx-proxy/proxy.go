...
// which for streaming MLX responses is dominated by token
...
const (
	...
	// workload this proxy produces. This proxy only ever fronts MLX.
	workloadEngine = "mlx"
)
...
// non-inference noise. MLX serves the OpenAI-compatible API, so these
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	slog.Debug("subscribing to discovery relay for routing targets", "service", string(noderec.ServiceMLX))
	if err := p.codec.Notify(noderec.MethodSubscribe, noderec.SubscribeParams{Services: []noderec.ServiceKey{noderec.ServiceMLX}}); err != nil {
	...
	// connection — so the target MLX sees the client disconnect and stops
...
func (retrySignal) Error() string { return "mlx-proxy: retry next candidate" }
...
// serveModelList queries every MLX candidate concurrently and returns
...
// (self-forward guard): a local MLX advertisement could otherwise
...
func (p *Proxy) resolveCandidates(model string) []candidate {
	...
	if model != "" {
		...
		resident := make([]Node, 0, len(nodes))
		for _, node := range nodes {
			if !nodeAdvertisesModel(node, model) {
				continue
			}
			owners = append(owners, node)
			if nodeHoldsModel(node, model) {
				resident = append(resident, node)
			}
		}
		// mlx_lm.server holds ONE model at a time: serving a different one
		// costs a full unload and reload of multi-gigabyte weights. So an
		// owner that already has this model resident takes the request, and an
		// owner that merely has it on disk is a candidate only when nobody is
		// resident.
		//
		// The fallback is the point. Routing to resident owners *alone* reads
		// as the stricter, thrash-free policy, but it deadlocks a cold
		// cluster: with nothing loaded anywhere, every node is ineligible and
		// the first request 502s on a system that is entirely healthy. Falling
		// back to disk owners means the first request pays one load and every
		// request after it lands on the node that already paid -- the cluster
		// converges on its own, with no lease, no warm-up job, and no
		// coordinator to designate a loader.
		if preferResidentOwners() && len(resident) > 0 {
			owners = resident
		}
		nodes = owners
	}
	...
	add := func(n Node) {
		...
		case isSelfTarget(u, selfPort):
			// Our own advertised endpoint (mx now points at this proxy). Serve
...
// routingPolicyEnv names the A/B switch for the one routing rule that differs
// from the proxy this one was cloned from. It exists so the residency
// preference can be measured against the inherited behaviour rather than
// asserted: set NVPAIR_MLX_ROUTING=any and mlx-proxy ranks owners exactly as
// ollama-proxy and lmstudio-proxy do, which is the control arm. Any other value
// (including unset) is the residency-preferring default.
//
// It is read per call rather than cached at startup so a benchmark can flip
// arms without a respawn, and because the read is a map lookup against a
// process-local copy of the environment -- far cheaper than the routing
// decision it guards.
const routingPolicyEnv = "NVPAIR_MLX_ROUTING"

func preferResidentOwners() bool {
	return os.Getenv(routingPolicyEnv) != "any"
}

// nodeHoldsModel reports whether the node has this model resident in memory
// right now, as opposed to merely downloaded. See resolveCandidates for why the
// two are ranked rather than filtered.
func nodeHoldsModel(n Node, model string) bool {
	if model == "" {
		return false
	}
	for _, loaded := range n.Loaded {
		if loaded == model {
			return true
		}
	}
...
func upstreamUnreachableID(nodeID string) string {
	return "mlx-proxy:upstream-unreachable:" + nodeID
}
...
// MLX port the broker's engine poller registered, not the proxy's listen
// port).
func subscribedToNode(n noderec.DirectoryNode) (Node, bool) {
	svc, ok := n.Services[noderec.ServiceMLX]
	if !ok || n.IP == "" {
		return Node{}, false
	}
	...
	return Node{
		...
		// Filter on this node's MLX models only, not the cross-engine union,
		// so a model a dual-engine node serves solely via Ollama isn't accepted as
		// an MLX owner here (falls back to the union for a peer that sends
		// no attribution — see DirectoryNode.EngineModels).
		Models: append([]string(nil), n.EngineModels("mlx")...),
		// Residency, reported by the peer's engine-manager from mlx-lm's
		// /health. A load in progress reports no model at all (mlx-lm sets its
		// model key only once the weights are in), so a node mid-load is
		// correctly not preferred rather than advertising a model it cannot
		// yet serve.
		Loaded: append([]string(nil), n.EngineLoaded("mlx")...),
	}, true
}
