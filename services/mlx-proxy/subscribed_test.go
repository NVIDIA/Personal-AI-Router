...
// node's MLX models only, never the cross-engine union.
func TestSubscribedToNode(t *testing.T) {
	...
	// Per-engine attribution: a dual-engine node projects ONLY its MLX
	...
	dual := noderec.DirectoryNode{
		...
		Name:     "host-d",
		IP:       "10.0.0.7",
		Models:   []string{"ollama-model", "mlx-model"},
	...
	if len(got.Models) != 1 || got.Models[0] != "mlx-model" {
		t.Fatalf("dual-engine projection Models = %v, want [mlx-model] only", got.Models)
	}
}
...
func TestSubscribedToNodeKeysByHostUUID(t *testing.T) {
	...
	n := noderec.DirectoryNode{
		...
		Name:     "host-a",
		IP:       "10.0.0.5",
		Services: map[noderec.ServiceKey]noderec.ServiceStatus{noderec.ServiceMLX: {Port: 1234}},
	}
