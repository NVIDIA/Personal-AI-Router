...
// cannot compute this tag, so a bystander who only learned the inviteId can no
// longer forge a terminal signal. (The Initial Exchange is unauthenticated, so
// an active on-path attacker who completes a Key Exchange can derive the key —
// active MITM is out of scope, see the README threat notes.) The tag is
// base64-encoded for transport; verification decodes before comparing.
// pairingMsgType returns the EAP-NOOB message Type of a raw wire message, or 0
// when it cannot be read. Used to spot a repeated Key Exchange (type 3) on a
// session that already holds its ephemeral secret.
func pairingMsgType(raw []byte) int {
	var probe struct {
		Type *int `json:"Type"`
	}
	if json.Unmarshal(raw, &probe) != nil || probe.Type == nil {
		return 0
	}
	return *probe.Type
}
