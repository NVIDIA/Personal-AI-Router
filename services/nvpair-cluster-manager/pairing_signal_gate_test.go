...
// TestSecondInitialExchangeRejected guards against re-keying an established
// joiner session: the EAP peer's onKeyExchangeRequest has no state guard, so a
// repeated Key Exchange on a session that already holds its ephemeral secret
// would overwrite that secret and let the poster forge the terminal-signal MAC.
// A Key Exchange on a keyed session must be refused and the secret unchanged.
func TestSecondInitialExchangeRejected(t *testing.T) {
	m := newTestManager(t)
	m.addSelfMember()
	...
	// Drive the session's Peer to Waiting (Key Exchange complete) against a
	// throwaway Server, so the session holds its ephemeral secret.
	server := newPairingServer(map[string]any{"role": "inviter"})
	msg, err := server.Start()
	if err != nil {
		t.Fatalf("server start: %v", err)
	}
	for {
		out, err := peer.Receive(msg)
		if err != nil || out.Err != nil {
			t.Fatalf("peer receive: err=%v protocolErr=%v", err, out.Err)
		}
		if len(out.Send) == 0 {
			break
		}
		sout, err := server.Receive(out.Send)
		if err != nil || sout.Err != nil {
			t.Fatalf("server receive: err=%v protocolErr=%v", err, sout.Err)
		}
		if len(sout.Send) == 0 {
			break
		}
	...
	before, err := peer.EphemeralKey()
	if err != nil {
		t.Fatalf("peer should hold a key after Key Exchange: %v", err)
	}
	...
	// A fresh Key Exchange request (Type 3) on the keyed session is refused.
	// The handler rejects on Type before the Peer ever parses it, so the
	// payload only needs to be a valid envelope.
	rekey := []byte(`{"Type":3}`)
	rec := postPairing(t, m, pairingEnvelope{
		InviteID: "inv-rekey", Phase: "initial",
		Msg: base64.StdEncoding.EncodeToString(rekey),
	})
	if rec.Code != http.StatusConflict {
		t.Fatalf("repeated key exchange status = %d, want 409", rec.Code)
	}
	after, err := peer.EphemeralKey()
	if err != nil {
		t.Fatalf("session key after refused re-key: %v", err)
	}
	if !bytes.Equal(before, after) {
		t.Fatal("repeated key exchange overwrote the session key")
	}
}
