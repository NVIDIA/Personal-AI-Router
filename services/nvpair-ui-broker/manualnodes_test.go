...
func TestManualNodeBridgesLlamaCppIntoProxy(t *testing.T) {
	...
	if call.method != "node/add-manual" {
		t.Fatalf("method = %q, want node/add-manual", call.method)
	}
...
func TestManualNodeRemovesLlamaCppFromProxyWhenDown(t *testing.T) {
	...
	call := readProxyManualCall(t, calls)
	if call.method != "node/remove-manual" {
		t.Fatalf("method = %q, want node/remove-manual", call.method)
	}
