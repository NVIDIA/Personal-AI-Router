...
func TestHealthChecksReuseConnections(t *testing.T) {
	test := func(name, path string, check func(*http.Client, int) bool) {
		t.Run(name, func(t *testing.T) {
			testFraming := func(name string, chunked bool) {
				t.Run(name, func(t *testing.T) {
					...
					const rounds = 32
					for i := 0; i < rounds; i++ {
						if got, want := check(client, 1), i%2 == 0; got != want {
	...
	test("ollama", "/", checkOllamaHealth)
	test("lmstudio", "/v1/models", checkLMStudioHealth)
}
