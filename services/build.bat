...
for /f "delims=" %%V in ('jq -r --arg k "nvpair-proxy"         ".components[$k]" "%VERSIONS_FILE%"')            do set "V_PROXY=%%V"
...
mkdir "%BIN_OUT%"
copy /y "%ROOT%nvpair-proxy\nvpair-proxy.exe" "%BIN_OUT%\nvpair-proxy.exe" >nul || goto :fail
