...
The services bundle needs nothing from the host but a terminal and a writable
data directory, so it also runs as a container, and as a pod on Kubernetes.
[Containers and Kubernetes](containers.mdx) has a Dockerfile, the manifests, and
the two things a container must get right: host networking for discovery and a
loopback relay for the endpoint.
