...
The unified ingress enforces the invariant where the backend is stored:
...
- `proxy_hardening_test.go` covers all three fixes: over-cap and at-cap
  bodies, the 413 response shape, loopback/non-loopback backend hosts, and
