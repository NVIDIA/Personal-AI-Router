...
flowchart TB
    ...
    EngineA2["<b>Engine</b><br/>LM Studio / vLLM"]
...
- An **engine** is an inference server on a node: Ollama, LM Studio, or vLLM. A
  node can
...
placed on. Every engine counts together, so Ollama load affects LM Studio and
vLLM ordering
and vice versa.
...
implicit `:latest` tag is normalized; LM Studio and vLLM model IDs match exactly.
...
**Every engine is counted as one pool.** Ollama, LM Studio, and vLLM load is
summed,
...
known install locations for each engine, so an Ollama, LM Studio, or vLLM you
installed
...
| `1234` | OpenAI-compatible proxy for LM Studio and vLLM (LM Studio moves to `1235`+) |
| `8000` | vLLM engine (adopted or PAIR-started; the proxy never takes this port) |
