...
1. Generate a key. Anything of at least 32 printable ASCII characters with no
   whitespace works; a random one is best:
...
2. Put it in the proxy key file on the node, one key per line (`#` starts a
   ...
   file other users can read is ignored, and the node's log says so.
...
so you can add, rotate, or revoke keys without restarting PAIR. Three environment
...
key. The node logs a warning when authenticated access is enabled and logs every
