...
├── .cursor/       Architecture and discipline rules, skills, and commit hooks
├── .agents/       Shared agent skills (UI patterns, backend sync, test style)
└── Makefile       Wraps the build and check commands (Linux and macOS)
```

`.cursor/rules/` is not advisory. Those files record the boundaries the code is
actually held to — process ownership, IPC shape, typing discipline, and where
state lives — and a change that violates one will be caught in review. Read the
rules covering the area you are touching before you start, and prefer
`services/*/spec.md` over a component README where the two disagree.
...
Twelve Go binaries. Each component is its own module, with its tests beside its
...
- [.cursor/rules/](.cursor/rules/) — the conventions this codebase enforces, and
  [.cursor/skills/pair-github-pr/](.cursor/skills/pair-github-pr/) for filling in
  a pull request's release-intent block
