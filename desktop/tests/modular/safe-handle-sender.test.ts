...
describe('safeHandle sender authorization', () => {
    ...
    it('authorizes a dev-URL sender when the configured URL has a trailing slash', async () => {
        process.env.ELECTRON_RENDERER_URL = 'http://localhost:5173/'
        safeHandle('test:sender-slash' as never, (() => 'ok') as never)
        await expect(
            invoke('test:sender-slash', 'http://localhost:5173/?window=tray')
        ).resolves.toEqual({
            success: true,
            data: 'ok'
        })
