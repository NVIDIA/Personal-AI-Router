...
!macro pairCloseRunningProcesses
  ...
  nsExec::ExecToLog 'taskkill /F /T /IM "llamacpp-proxy.exe"'
...
!macro pairAddFirewallRules
  ...
  nsExec::ExecToLog 'netsh advfirewall firewall add rule name="Personal AI Router llama.cpp Proxy" dir=in action=allow program="$INSTDIR\resources\cli-bin\llamacpp-proxy.exe" enable=yes profile=any remoteip=localsubnet'
  ...
  nsExec::ExecToLog 'netsh advfirewall firewall add rule name="Personal AI Router mDNS llama.cpp Proxy (UDP 5353)" dir=in action=allow protocol=UDP localport=5353 program="$INSTDIR\resources\cli-bin\llamacpp-proxy.exe" enable=yes profile=any remoteip=localsubnet'
