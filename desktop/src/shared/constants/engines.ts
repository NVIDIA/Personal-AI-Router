...
export const EngineTypes = ['ollama', 'lm-studio', 'mlx'] as const

// Kept as a distinct export so a future engine can ship behind it rather than
// appearing the moment its type exists.
export const EnabledEngineTypes: EngineType[] = ['ollama', 'lm-studio', 'mlx'] as const
...
export const EngineDefaultLinks: Record<EngineType, { docsUrl: string; installUrl: string }> = {
    ...
    // MLX has no installer to link to: PAIR builds the engine itself, as a
    // Python environment (see services/nvpair-engine-manager/manifests/mlx.json).
    mlx: {
